...
<body class="d-flex align-items-center py-4 bg-body-tertiary">
	...
	<main class="form-signin w-100 m-auto">
		<form action="adminJoin" method="post"
			...
			<div class="form-floating">
				<input type="text" class="form-control adId" id="floatingInput adId" name="adId" placeholder="아이디"> <label
					for="floatingInput">아이디</label> <span id="idCheckMsg"></span>
			</div>
			<button class="btn btn-secondary w-100" type="button"
				onclick="adminIdCheck('${pcrCode}')">중복확인</button>
			<div class="form-floating">
				<input type="password" class="form-control" id="floatingPassword adPw"
					name="adPw" placeholder="비밀번호"> <label
					for="floatingPassword">비밀번호</label> <span id="pwCheckMsg"></span>
			</div>

			<div class="form-floating">
				<input type="text" class="form-control" id="floatingInput adName"
					name="adName" placeholder="이름"> <label for="floatingInput">이름</label>
				<span id="nameCheckMsg"></span>
			</div>

			<div class="form-floating">
				<input type="text" class="form-control" id="floatingInput adEmail"
					name="adEmail" placeholder="이메일"> <label
					for="floatingInput">이메일</label> <span id="emailCheckMsg"></span>
			</div>

			<div class="form-floating">
				<input type="text" class="form-control" id="floatingInput adHp"
					name="adHp" placeholder="전화번호"> <label for="floatingInput">전화번호</label>
				<span id="hpCheckMsg"></span>
			</div>
			<div style="display:none;">
				<input type="text" name="pcrCode" value="${pcrCode}">
			</div>
			
			<button class="btn btn-primary w-100 py-2" type="submit">관리자 회원가입</button>
	...
	const hpMsgEl = document.querySelector('#hpCheckMsg');
		...
		function adminIdCheck(pcrCode) {
			console.log('adminIdCheck() 호출');
			// 문서에서 아이디를 입력하는 input 태그를 선택
			let inputAdidEl = document.querySelector('.adId');
			console.log(inputAdidEl);
			$.ajax({
				type : 'post',
				url : 'adIdCheck',
				data : {'adId' : inputAdidEl.value,'pcrCode':pcrCode},
		...
		function formCheck(formObj) {
			...
			console.log(formObj.adId);
			/* 아이디 입력 태그*/ 
			 let idEl = formObj.adId;
