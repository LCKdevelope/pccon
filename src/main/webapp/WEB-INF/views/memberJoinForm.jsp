...
<body class="d-flex align-items-center py-4 bg-body-tertiary">
	...
	<main class="form-signin w-100 m-auto">
		<form action="memberJoin" method="post"
			...
			<div class="form-floating">
				<input type="text" class="form-control memId"
					id="floatingInput memId" name="memId" placeholder="아이디"> <label
					for="floatingInput">아이디</label> <span id="idCheckMsg"></span>
			</div>
			<button class="btn btn-secondary w-100" type="button"
				onclick="memberIdCheck()">중복확인</button>
			<div class="form-floating">
				<input type="password" class="form-control"
					id="floatingPassword memPw" name="memPw" placeholder="비밀번호">
				<label for="floatingPassword">비밀번호</label> <span id="pwCheckMsg"></span>
			</div>

			<div class="form-floating">
				<input type="text" class="form-control" id="floatingInput memName"
					name="memName" placeholder="이름"> <label for="floatingInput">이름</label>
				<span id="nameCheckMsg"></span>
			</div>

			<div class="form-floating">
				<input type="text" class="form-control" id="floatingInput memEmail"
					name="memEmail" placeholder="이메일"> <label
					for="floatingInput">이메일</label> <span id="emailCheckMsg"></span>
			</div>

			<div class="form-floating">
				<input type="text" class="form-control" id="floatingInput memHp"
					name="memHp" placeholder="전화번호"> <label for="floatingInput">전화번호</label>
				<span id="hpCheckMsg"></span>
			</div>

			<div class="form-floating">
				<input type="text" class="form-control" id="floatingInput memAge"
					name="memAge" placeholder="나이"> <label for="floatingInput">나이</label>
				<span id="ageCheckMsg"></span>
			</div>
			<div class="form-floating" style="display: none;">
				<input type="text" value="${pcrCode}" class="form-control" id="pcrCode" name="pcrCode" > 
	...
	const idMsgEl = document.querySelector('#idCheckMsg');
	...
	const ageMsgEl = document.querySelector('#ageCheckMsg');
		...
		function memberIdCheck() {
			...
			// 문서에서 아이디를 입력하는 input 태그를 선택
			let inputMidEl = document.querySelector('.memId');
			console.log(inputMidEl.value);
			$.ajax({
				type : 'post',
				url : 'memberIdCheck',
				data : {'memId' : inputMidEl.value,'pcrCode':'${pcrCode}'},
				success : function(result) {
					console.log('중복확인 결과 : ' + result);
					if (result == 'Y') {
						idMsgEl.innerText = '사용 가능한 아이디';
						idMsgEl.style.color = 'green';
						idcheck=true;
						

					} else {
						idMsgEl.innerText = '이미 사용중인 아이디';
		...
		function formCheck(formObj) {
			...
			console.log(formObj.memId);
			/* 아이디 입력 태그*/ 
			 let idEl = formObj.memId;
			 let pwEl = formObj.memPw;
			 let NameEl = formObj.memName;
			 let EmailEl = formObj.memEmail;
			 let HpEl = formObj.memHp;
			 let AgeEl = formObj.memAge;
			 
			if (idEl.value.length == 0) {
				
				idMsgEl.innerText = '아이디를 입력 해주세요!';
				idMsgEl.style.color = 'red';
				idEl.focus();
				return false;
